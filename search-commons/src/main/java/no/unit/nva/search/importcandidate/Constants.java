...
/**
 * Constants for the Import Candidates.
 *
 * @author Stig Norland
 */
public final class Constants {
    ...
    return nestedBranchBuilder(CONTRIBUTOR, CONTRIBUTORS)
        .subAggregation(
            branchBuilder(ID, CONTRIBUTORS, IDENTITY, ID, KEYWORD)
                .subAggregation(branchBuilder(NAME, CONTRIBUTORS, IDENTITY, NAME, KEYWORD)));
  }
  ...
  public static NestedAggregationBuilder topLevelOrganisationsHierarchy() {
    return nestedBranchBuilder(ORGANIZATIONS, ORGANIZATIONS)
        .subAggregation(
            branchBuilder(ID, ORGANIZATIONS, ID, KEYWORD).subAggregation(labels(ORGANIZATIONS)));
  }
  ...
  public static Script selectByLicense(String license) {
    ...
    return new Script(
        ScriptType.INLINE,
        no.unit.nva.search.resource.Constants.PAINLESS,
        script,
