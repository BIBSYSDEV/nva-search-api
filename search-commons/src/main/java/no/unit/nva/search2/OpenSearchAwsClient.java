...
public class OpenSearchAwsClient implements OpenSearchClient<SearchResponse, ResourceAwsQuery> {
    ...
    public OpenSearchAwsClient(CachedJwtProvider cachedJwtProvider) {
        super();
    ...
    public static OpenSearchAwsClient defaultClient() {
        ...
        return new OpenSearchAwsClient(cachedJwtProvider);
    }
    ...
    private static int getFrom(ResourceAwsQuery query) {
        return Integer.parseInt(query.getValue(FROM));
    }

    private static int getSize(ResourceAwsQuery query) {
    ...
    private SearchResponse searchResponse(SearchRequest searchRequest) {
        try (RestHighLevelClient client = new RestHighLevelClient(
            RestClient.builder(HttpHost.create(SEARCH_INFRASTRUCTURE_API_URI))
        )) {
