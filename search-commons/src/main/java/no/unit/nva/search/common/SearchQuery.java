...
/**
 * This class operates on the search query that a request provides.
 *
 * @author Stig Norland
 */
...
public abstract class SearchQuery<K extends Enum<K> & ParameterKey<K>> extends Query<K> {
  ...
  public <R, Q extends Query<K>> ResponseFormatter<K> doSearch(OpenSearchClient<R, Q> queryClient) {
    final var source =
        URI.create(getNvaSearchApiUri().toString().split(PATTERN_IS_URL_PARAM_INDICATOR)[0]);
  ...
  private void handleSorting(SearchSourceBuilder builder) {
    if (isSortByRelevance()) {
      // Not very well documented. This allows sorting on relevance and other fields.
