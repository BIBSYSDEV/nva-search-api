...
public enum ResourceParameterKey implements ParameterKey {
    ...
    CREATED_BEFORE(SHORT_DATE, LESS_THAN, "createdDate"),
    CREATED_SINCE(SHORT_DATE, GREATER_THAN_OR_EQUAL_TO, "createdDate"),
    ...
    private KeyEncoding getEncoding(ParamKind kind) {
        return switch (kind) {
            case SHORT_DATE, NUMBER, CUSTOM -> KeyEncoding.NONE;
            case STRING, SORT_STRING -> KeyEncoding.DECODE;
        };
    }
    ...
    private String getValuePattern(ParamKind kind, String pattern) {
        return switch (kind) {
            ...
            case SHORT_DATE -> PATTERN_IS_DATE;
