...
import static no.unit.nva.search2.aws.Aggregations.AGGREGATIONS_MAP;
import static no.unit.nva.search2.constants.ApplicationConstants.SEARCH_INFRASTRUCTURE_API_HOST;
import static no.unit.nva.search2.constants.ApplicationConstants.SEARCH_INFRASTRUCTURE_AUTH_URI;
...
@JacocoGenerated
public class OpenSearchClient implements SearchClient<Publication> {
    ...
    @Override
    public SearchResponse<Publication> search(Query query) throws IOException {
        return client.withTransportOptions(getOptions())
                   .search(constructSearchRequest(query), Publication.class);
    }
    ...
    private static DeleteRequest contructDeleteRequest(Publication indexDocument) {
        return new DeleteRequest.Builder()
                   .index(NVI_CANDIDATES_INDEX)
                   .id(indexDocument.getIdentifier().toString())
                   .build();
    }

    private static IndexRequest<Publication> constructIndexRequest(
        Publication indexDocument) {
        return new IndexRequest.Builder<Publication>()
                   .index(NVI_CANDIDATES_INDEX)
                   .id(indexDocument.getIdentifier().toString())
