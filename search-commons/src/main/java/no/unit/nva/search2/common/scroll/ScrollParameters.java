...
public enum ScrollParameters implements ParameterKey {
    ...
    ORCID(KEYWORD, ALL_ITEMS, CONTRIBUTORS_IDENTITY_ORC_ID_KEYWORD),
    ...
    public static final Set<ScrollParameters> RESOURCE_PARAMETER_SET =
        Arrays.stream(ScrollParameters.values())
            .filter(ScrollParameters::isSearchField)
    ...
    ScrollParameters(ParameterKind kind, String fieldsToSearch) {
        this(kind, ALL_ITEMS, fieldsToSearch, null, null, null);
    }

    ScrollParameters(ParameterKind kind, String fieldsToSearch, Float boost) {
        this(kind, ALL_ITEMS, fieldsToSearch, null, null, boost);
    }

    ScrollParameters(ParameterKind kind, FieldOperator operator, String fieldsToSearch) {
        this(kind, operator, fieldsToSearch, null, null, null);
    }

    ScrollParameters(ParameterKind kind, FieldOperator operator, String fieldsToSearch, Float boost) {
    ...
    public static ScrollParameters keyFromString(String paramName) {
        var result = Arrays.stream(ScrollParameters.values())
            .filter(ScrollParameters::ignoreInvalidKey)
    ...
    private static boolean ignoreInvalidKey(ScrollParameters enumParameter) {
        return enumParameter.ordinal() > IGNORE_PARAMETER_INDEX;
    }

    private static boolean isSearchField(ScrollParameters enumParameter) {
