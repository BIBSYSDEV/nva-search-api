...
public final class TicketSearchQuery extends SearchQuery<TicketParameter> {
  ...
  private Stream<Entry<TicketParameter, QueryBuilder>> builderStreamByAssignee() {
    var searchByUserName =
        parameters().isPresent(BY_USER_PENDING) // override assignee if <user pending> is used
  ...
  /** Validator for Ticket search parameters. */
  public static class TicketParameterValidator
    ...
    protected void applyRulesAfterValidation() {
      ...
      if (searchQuery.parameters().isPresent(BY_USER_PENDING)) {
        searchQuery
            .parameters()
            .set(TicketParameter.TYPE, searchQuery.parameters().get(BY_USER_PENDING).as());
    ...
    protected void setValue(String key, String value) {
      ...
      var decodedValue = qpKey.valueEncoding() != ValueEncoding.NONE ? decodeUtf(value) : value;
      switch (qpKey) {
