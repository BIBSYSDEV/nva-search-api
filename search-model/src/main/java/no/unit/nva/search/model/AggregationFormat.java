...
import static no.unit.nva.search.model.AggregationFormat.Constants.DOC_COUNT;
import static no.unit.nva.search.model.constant.Patterns.PATTERN_IS_WORD_ENDING_WITH_HASHTAG;
import static no.unit.nva.search.model.constant.Words.BUCKETS;
import static no.unit.nva.search.model.constant.Words.ENGLISH_CODE;
import static no.unit.nva.search.model.constant.Words.KEY;
...
public final class AggregationFormat {
    ...
    private static JsonNode fixNodes(JsonNode node) {
        if (node.isArray()) {
            ...
            node.fields()
                    .forEachRemaining(entry -> extractKeyAndLabels(entry, outputAggregationNode));
    ...
    private static void extractKeyAndLabels(
        ...
        if (keyIsLabel(entry)) {
            ...
            outputAggregationNode.set(
                    DOC_COUNT, entry.getValue().get(UNIQUE_PUBLICATIONS).get(VALUE));
        } else {
    ...
    private static Map.Entry<String, JsonNode> getJsonNodeEntry(Map.Entry<String, JsonNode> entry) {
        return Map.entry(
                getNormalizedFieldName(entry.getKey()), getBucketOrValue(entry.getValue()));
    }
    ...
    private static boolean keyIsName(Entry<String, JsonNode> entry) {
        return Words.NAME.equals(entry.getKey());
    }
