...
public class AggregationIntegrationTest {
    ...
    void setUp() {
        ...
        var cachedJwt = setupMockedCachedJwtProvider();

        searchClient = new SearchClient(restHighLevelClientWrapper, cachedJwt);
        indexingClient = new IndexingClient(restHighLevelClientWrapper, cachedJwt);
    ...
    void aggregatesOnBidragsyter() throws IOException, InterruptedException {
        ...
        var b = response.getAggregations();
    ...
    @Test
    void sddsf() throws IOException, InterruptedException {
        var indexDocument = createSampleIndexDocument("publication.json");

        indexingClient.addDocumentToIndex(indexDocument);
        Thread.sleep(DELAY_AFTER_INDEXING);

        var oldResponse = aggregationClient.aggregate(Map.of(
                "sdfdfsa", "entityDescription.contributors.identity.name.keyword",
                "asdfffweew", "entityDescription.reference.publicationInstance.type.keyword"
        ));
        ...
        var aggregation = response.getAggregations().asMap();
        assertThat(aggregation.size(), is(equalTo(2)));
        assertThat(aggregation.get("Bidragsyter").getName(), is(equalTo("Bidragsyter")));
    }
