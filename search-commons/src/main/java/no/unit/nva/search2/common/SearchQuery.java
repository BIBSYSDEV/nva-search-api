...
import no.unit.nva.search2.common.builder.QueryBuilderAcrossFields;
import no.unit.nva.search2.common.builder.QueryBuilderFuzzyKeyword;
...
import no.unit.nva.search2.common.builder.PagedSearchBuilder;
...
public abstract class SearchQuery<K extends Enum<K> & ParameterKey> extends Query<K> {
    ...
    protected Stream<Entry<K, QueryBuilder>> builderStreamDefaultQuery(K key) {
        ...
        return switch (key.fieldType()) {
            case DATE, NUMBER -> new QueryBuilderRange<K>().buildQuery(key, value);
            case KEYWORD -> new QueryBuilderKeyword<K>().buildQuery(key, value);
            case FUZZY_KEYWORD -> new QueryBuilderFuzzyKeyword<K>().buildQuery(key, value);
            case TEXT -> new QueryBuilderText<K>().buildQuery(key, value);
            case FREE_TEXT -> queryTools.queryToEntry(key, builderSearchAllQuery(key));
            case ACROSS_FIELDS -> new QueryBuilderAcrossFields<K>().buildQuery(key, value);
