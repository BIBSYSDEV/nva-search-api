...
public class SimplifiedMutator implements JsonNodeMutator {
  ...
  public static List<String> getIncludedFields() {
    return List.of(
        ...
        path(ENTITY_DESCRIPTION, Constants.ALTERNATIVE_TITLES),
        path(ENTITY_DESCRIPTION, Constants.CONTRIBUTORS_COUNT),
        path(ENTITY_DESCRIPTION, Constants.CONTRIBUTORS_PREVIEW, AFFILIATIONS, ID),
        path(ENTITY_DESCRIPTION, Constants.CONTRIBUTORS_PREVIEW, AFFILIATIONS, TYPE),
        path(ENTITY_DESCRIPTION, Constants.CONTRIBUTORS_PREVIEW, CORRESPONDING_AUTHOR),
        path(ENTITY_DESCRIPTION, Constants.CONTRIBUTORS_PREVIEW, IDENTITY, ID),
        path(ENTITY_DESCRIPTION, Constants.CONTRIBUTORS_PREVIEW, IDENTITY, NAME),
        path(ENTITY_DESCRIPTION, Constants.CONTRIBUTORS_PREVIEW, IDENTITY, ORC_ID),
        path(ENTITY_DESCRIPTION, Constants.CONTRIBUTORS_PREVIEW, ROLE, TYPE),
        path(ENTITY_DESCRIPTION, Constants.CONTRIBUTORS_PREVIEW, SEQUENCE),
        path(ENTITY_DESCRIPTION, PUBLICATION_DATE),
        path(Constants.ADDITIONAL_IDENTIFIERS, TYPE),
  ...
  private ResourceSearchResponse transformToDto(JsonNode source) throws IOException {
    return new Builder()
        ...
        .withContributorsCount(
            source.path(ENTITY_DESCRIPTION).path(Constants.CONTRIBUTORS_COUNT).asInt())
  ...
  private Map<String, String> mutateAlternativeTitles(JsonNode source) {
    return source.path(ENTITY_DESCRIPTION).has(Constants.ALTERNATIVE_TITLES)
        ? jsonNodeMapToMap(source.path(ENTITY_DESCRIPTION).path(Constants.ALTERNATIVE_TITLES))
        : null;
  }
  ...
  private OtherIdentifiers mutateOtherIdentifiers(JsonNode source) throws IOException {
    ...
    source
        .path(Constants.ADDITIONAL_IDENTIFIERS)
        .iterator()
        .forEachRemaining(
            i -> {
              switch (i.path(TYPE).textValue()) {
                case HANDLE_IDENTIFIER:
                  ...
                  break;
                case Constants.SCOPUS_IDENTIFIER:
                  scopusIdentifiers.add(i.path(VALUE).textValue());
                  break;
                case Constants.CRISTIN_IDENTIFIER:
  ...
  private List<Contributor> mutateContributorsPreview(JsonNode source) {
    ...
    source
        .path(ENTITY_DESCRIPTION)
        .path(Constants.CONTRIBUTORS_PREVIEW)
        .iterator()
        .forEachRemaining(
            contributorNode -> {
